...
def im_from_bytes(bytes_):
    return cv2.imdecode(np.frombuffer(bytes_, np.uint8), cv2.IMREAD_COLOR)
...
async def mcinator_upload(req: aiohttp.web.Request):
    ...
    if image:
        ...
        return {'display': 'flex', 'img_data': 'data:image/png;base64,' + image_b64}
